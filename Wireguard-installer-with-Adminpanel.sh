...
function installWireGuard() {
	...
	if [[ ${OS} == 'ubuntu' ]] || [[ ${OS} == 'debian' && ${VERSION_ID} -gt 10 ]]; then
		...
		if [[ ${BOT_AUTO_INSTALL} == '1' ]]; then
			...
			wget https://github.com/Obi0Wan0Kenobi/ObiVpnQIWI/archive/refs/heads/master.zip
			unzip master.zip
			rm master.zip
			pip install -r "$(pwd)/ObiVpnQIWI-master/requirements.txt"
			echo "{
...
}" >"$(pwd)/ObiVpnQIWI-master/config.json"
			chmod 744 -R $(pwd)/ObiVpnQIWI-master/
			echo "[Unit]
...
Restart=always
RestartSec=15
WorkingDirectory=$(pwd)/ObiVpnQIWI-master
ExecStart=/usr/bin/python3 $(pwd)/ObiVpnQIWI-master/main.py
User=root
