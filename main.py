...
async def AddTimeToUser(tgid,timetoadd):
    ...
    if int(userdat.subscription) < int(time.time()):
        ...
        await bot.send_message(userdat.tgid, e.emojize( 'Данные для входа были обновлены, скачайте новый файл авторизации через раздел "Как подключить :gear:"'))
    else:
                ...
                for i in log:
                    ...
                    if status=="PAID":
                        ...
                        if int(userdat[2])<int(time.time()):
                            ...
                            BotChecking.send_message(i['tgid'],e.emojize('Данные для входа были обновлены, скачайте новый файл авторизации через раздел "Как подключить :gear:"'))
                        else:
